...
//
// --> TODO::: !!!!!!!!
// linea 241: añadir un argumento a las funciones externas, para reenviar al SSWorld en caso de SpatDifPlayer_play (para tener feedback visual)

// IMPLEMENTAR SOURCE TYPE
//
////////////////////////////////////////////////////////////////////////////
...
SpatialRender {
	...
	var defaultBinauralDecoder = \listen;
	var defaultSubjectID = 1002;
	var <binauralDecoder;
	...
	*new{ | server, oscLoggerPath |
		^super.new.init(server,oscLoggerPath);
	}

	init { |myServer, myOscLoggerPath|
		...
		server.options.numOutputBusChannels_(24);
		...
		// // // // INIT OSC LOGGER // // // //
		this.initOSCLogger(myOscLoggerPath);
	...
	mediaType { |name, type|
		if (type == \jack) {
			...
			if (sourceNames.indexOf(name).isNil) {
				...
				{\binaural} {
					var synthName = \vbapEncoder;
					pannerSynth = Synth(synthName,[\busIn,newPannerBus,\subjectID,binauralSubjectID],target:newSourceGroup,addAction:\addToTail);
				}
				;
	...
	setSpatializationTechnique { |newSpatialTechnique,internal=true|
		...
		if (internal) {
			...
			if (lastSpatialTechnique != newSpatialTechnique) {
				...
				this.setAudioOutput(spatialTechnique);
